$(document).ready(function() {
    ...
    function updateFreeSpotsCount() {
        $.getJSON('/get_free_spots_count', function(data) {
            $('#free-spots-count').text('Free Spots: ' + data.free_count);  // Update the text with the count of free spots
        }).fail(function() {
    ...
    function updateParkingStatus() {
        $.getJSON('/get_parking_status', function(statuses) {
            ...
            for (var label in statuses) {
                ...
                spot.removeClass('occupied reserved free').addClass(spotInfo.status);

                if (spotInfo.status === 'occupied') {
                    var timeParkedFormatted = formatTime(spotInfo.time_parked);
                    timerSpan.text(timeParkedFormatted);
                } else if (spotInfo.status === 'reserved occupied' || spotInfo.status === 'reserved empty') {
                    var timeLeftFormatted = formatTime(spotInfo.time_left);
                    timerSpan.text(timeLeftFormatted);
                } else {
                    timerSpan.text('-');
                }
                ...
                } else if (spotInfo.status === 'reserved occupied') {
                    spot.css('background-color', '#1515c6');
                } else if (spotInfo.status === 'reserved empty') {
                    spot.css('background-color', '#008080'); 
                } else {
...
function highlightNearestFreeSpot(nearestSpotId) {
    ...
    console.log('Class applied:', nearestSpot.hasClass('nearest-free-spot'));  // This should log true if class is applied
}
